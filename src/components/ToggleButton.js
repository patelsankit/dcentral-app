...
const ToggleButton = () => {
  ...
  const toggleDarkMode = (value) => {
    ...
    if (value) {
      ...
      localStorage.theme = "dark"; // Save user preference to localStorage
    } else {
      body.classList.remove("dark");
      localStorage.theme = "light"; // Save user preference to localStorage
    }
  };

  // Function to respect OS preference
  const respectOSPreference = () => {
    const body = document.body;
    body.classList.remove("dark");
    localStorage.removeItem("theme"); // Remove user preference from localStorage
  };
  ...
  useEffect(() => {
    ...
    if (currentTheme === "dark" || (!currentTheme && prefersDarkMode)) {
      toggleDarkMode(true); // Set "dark" class initially
